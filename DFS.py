...
class Ui_Form(QtGui.QWidget):
    def setupUi(self, Form):
        ...
        self.prompt3.setText(u"正在遍历的点背景是绿色的，已被访问的点边是红色的")
        ...
        self.scene = SceneArcPaint.SceneArcPaint()
        ...
        self.timeController.frameChanged.connect(self.DFSProcess)

        self.NewsButton.clicked.connect(self.NewGraph)
        self.StartButton.clicked.connect(self.startDFS)

    def startDFS(self):
        Apex = self.TextData.toPlainText()
        self.scene.restartDfs()
        if len(Apex) != 1 :
    ...
    def DFSProcess(self):
        self.process = self.scene.DFS(self.process)
        if self.process[0] == 'stop':
            self.timeController.stop()
            message = QtGui.QMessageBox.warning(self, u"完成", u"深度优先遍历完成！", QtGui.QMessageBox.Yes, 0)

    def retranslateUi(self, Form):
        Form.setWindowTitle(_translate("Form", "Form", None))
        self.label.setText(_translate("Form", "图的深度优先遍历", None))
